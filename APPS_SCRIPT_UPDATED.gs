...
//  Handles: Email sending, Event tracking, Proposal storage, Client creation
// ═══════════════════════════════════════════════════════════

const SHEET_ID = "1U-kaTF-TEAd835RQVnZd4aCH5c9Wx9PVrzsENaJRtog";
const TRACKING_TAB = "tracking";
const PROPOSALS_TAB = "proposals";
const CLIENTS_GID = 0; // Main sheet (gid=0) holds clients
...
// ── Add a new client to the main clients sheet ────────────
function addClient(clientId, company, industry, email, size, notes) {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  // Main sheet is the first sheet (gid=0)
  const sheet = ss.getSheets()[0];
  const rows = sheet.getDataRange().getValues();

  // Check if header row exists and matches expected format
  // Expected: client id | company name | email | industry | size | notes
  if (rows.length === 0) {
    sheet.appendRow(["client id", "company name", "email", "industry", "size", "notes"]);
  }

  // Check for duplicate client_id
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][0]).trim() === String(clientId).trim()) {
      return jsonResponse({ success: false, error: "Client ID already exists: " + clientId });
    }
  }
  ...
  return jsonResponse({ success: true, client_id: clientId, company: company });
}

// ── Get highest existing FRIST number for auto-increment ──
function getNextClientId() {
  ...
  const next = maxNum + 1;
  const nextId = 'FRIST' + String(next).padStart(3, '0');
  return jsonResponse({ next_id: nextId, last_num: maxNum });
}
